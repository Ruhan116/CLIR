...
OUT_FILE = os.path.join(BASE_DIR, "dhakapost_stories.txt")
LINKS_FILE = os.path.join(BASE_DIR, "dhakapost_links")
...
def extract_date_from_jsonld(jsonld: dict) -> Optional[str]:
    ...
    for key in ("datePublished", "dateCreated", "dateModified", "date"):
        ...
        if isinstance(v, dict):
            # sometimes value can be an object with `@value` or `name`
...
def longest_p_block(html_text: str) -> str:
    # find all <p>...</p> spans and group contiguous ones (small gaps)
    ...
    for a, b in zip(matches, matches[1:]):
        gap = b.start() - a.end()
        if gap < 400:  # small gap -> likely contiguous article paragraphs
            cur_group.append(b)
        else:
    ...
    for grp in groups:
        ...
        # strip scripts/ads by removing tags except p
...
def extract_body_from_dom(soup: BeautifulSoup, raw_html: str) -> str:
    ...
    # fallback: look for longest block of consecutive <p> tags in raw HTML
    block = longest_p_block(raw_html)
    if block and len(block) > 200:
        return block

    # final fallback: collect all <p> but prefer longer ones
...
def extract_article(url: str) -> Optional[dict]:
    ...
    # extract publish date (JSON-LD preferred, then meta/time fallbacks)
